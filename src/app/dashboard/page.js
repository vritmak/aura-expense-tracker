        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
          <div className="md:col-span-2 bg-indigo-600 p-8 rounded-[2.5rem] text-white shadow-xl shadow-indigo-100 flex flex-col justify-between group cursor-pointer hover:bg-indigo-700 transition-all" onClick={() => setIsModalOpen(true)}>
            <div>
               ...
               <h2 className="text-4xl font-black tracking-tighter">₹{analytics.currentMonthTotal.toLocaleString('en-IN')}</h2>
               <p className="opacity-70 font-bold uppercase text-[10px] tracking-[0.2em] mt-1 italic">Total for this month</p>
            </div>
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
            ...
            <h3 className="text-2xl font-black tracking-tighter text-slate-800">₹{analytics.highestMonthTotal.toLocaleString('en-IN')}</h3>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1">All-Time Peak</p>
          </div>

          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
            <div className="bg-emerald-50 w-fit p-3 rounded-2xl text-emerald-600 mb-4"><BarChart3 size={20}/></div>
            <h3 className="text-2xl font-black tracking-tighter text-slate-800">₹{analytics.avgMonthTotal.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</h3>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1">Monthly Avg</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm h-[500px] flex flex-col">
            <div className="flex justify-between items-center mb-8">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-black text-sm uppercase tracking-[0.1em] text-slate-400">Current Month Breakdown</h3>
              </div>
            </div>
            ...
            {analytics.currentMonthExpenses.length > 0 ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center gap-4 text-slate-300">
                 ...
                 <p className="italic text-sm font-medium">No transactions this month.</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col h-[500px]">
            <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-white/80 backdrop-blur-md sticky top-0 z-10">
              ...
              <span className="text-[10px] font-black bg-indigo-50 px-3 py-1 rounded-full text-indigo-600 uppercase tracking-tighter">{expenses.length} Records</span>
            </div>
            <div className="flex-1 overflow-y-auto divide-y divide-slate-50 px-4 pb-4">
              {expenses.map((item) => (
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[3rem] p-12 shadow-2xl relative animate-in fade-in zoom-in duration-300">
            ...
            <h2 className="text-3xl font-black tracking-tighter mb-10">New Entry</h2>
            <form onSubmit={addExpense} className="space-y-6">
              <input required value={name} onChange={(e) => setName(e.target.value)} type="text" placeholder="Merchant Name" className="w-full p-5 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-600 transition outline-none font-bold text-slate-800 shadow-inner" />
              <input required value={amount} onChange={(e) => setAmount(e.target.value)} type="number" step="0.01" placeholder="Amount (₹)" className="w-full p-5 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-600 transition outline-none font-black text-slate-800 text-2xl shadow-inner" />
              <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full p-5 bg-slate-50 border-none rounded-2xl outline-none font-bold text-slate-800 appearance-none shadow-inner">
